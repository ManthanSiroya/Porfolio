...
function __OriginkitBase_Typewriter(props) {
    ...
    const [isWaiting, setIsWaiting] = useState((props.startDelay ?? 0) > 0)
        ...
        const startTyping = () => {
            if (isDeleting) {
                ...
                if (currentIndex < currentText.length) {
                    const isFirstChar = currentIndex === 0 && displayText === "";
                    
                    let extraDelay = 0;
                    let skipChars = 0;
                    if (currentText.slice(currentIndex).startsWith('<d:')) {
                        const endIdx = currentText.indexOf('>', currentIndex);
                        if (endIdx !== -1) {
                            const delayStr = currentText.slice(currentIndex + 3, endIdx);
                            extraDelay = parseInt(delayStr, 10) || 0;
                            skipChars = (endIdx - currentIndex) + 1;
                        }
                    }

                    let delay = typeDelayMs;
                    if (isFirstChar) delay += (startDelay ?? 0) * 1000;
                    delay += extraDelay;
                    
                    timeout = setTimeout(() => {
                        if (isWaiting) setIsWaiting(false);
                        if (skipChars > 0) {
                            setCurrentIndex((prev) => prev + skipChars);
                        } else {
                            setDisplayText(
    ...
    const currentText = list[currentTextIndex] ?? ""
    const isPaused = currentText.slice(currentIndex).startsWith('<d:');
    ...
    const stripTags = (str) => str.replace(/<d:\d+>/g, "");
    const renderedText = isStatic ? stripTags(list[0] ?? "") : displayText
    ...
    return (
        ...
        >
            ...
            >
                ...
                {showCursor && !isFinished && !isWaiting &&
                    (isStatic || isCanvas ? (
                        <span
                            style={{
